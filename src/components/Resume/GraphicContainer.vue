...
<script setup>
import { computed, defineProps, toRefs, ref, defineEmits, watch } from "vue";
...
const amountToPixels = (amount) => {
  const min = Math.min(...amounts.value); /// = 0
  const max = Math.max(...amounts.value); /// = 200
...
watch(pointer, (value) => {
  ...
  if (index < 0 || index > amounts.value.length) {
    emit("selection", amounts.value[index - 1]);
  }
});
